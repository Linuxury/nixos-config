...
mkdir -p "$VAULT/04 📋 Sessions" 2>/dev/null || true
...
write_obsidian_rename_warning() {
  local warnings="$1"
  local pending_file="$VAULT/00 📌 Quick Access/PENDING.md"

  # Append to PENDING.md rather than creating a separate file
...
# ---------------------------------------------------------------------------
# Write Obsidian session log entry
# ---------------------------------------------------------------------------
write_obsidian_log() {
  local status="$1"
  local log_file="$VAULT/04 📋 Sessions/${DATE} - $(date '+%H%M') - nixos-update-${status}.md"
  local icon="✅"
  local error_section=""
...
time: ${TIME}
ai: system
tags: [auto-update$([ "$status" = "failed" ] && echo ", error")]
---
...
${error_section}
## Related
- Project: \`05 🚀 Projects/NixOS-Config.md\`
EOF
}
...
write_obsidian_pending() {
  ...
  # Append failure item to PENDING.md
  cat >> "$VAULT/00 📌 Quick Access/PENDING.md" << EOF

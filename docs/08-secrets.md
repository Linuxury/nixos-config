...
> The CLI is **ragenix** — use `nix run nixpkgs#ragenix`. Do not use `nix run github:ryantm/agenix`.
> Always run ragenix from the `secrets/` directory — it reads `secrets.nix` from the current directory.

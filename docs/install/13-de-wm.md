...
> Do not edit COSMIC config files manually while COSMIC is running. Config files are in RON (Rusty Object Notation) format. Writing to them while `cosmic-session` holds them open causes parse errors and exponential backoff restart loops. Use Home Manager to write them declaratively, then rebuild.
...
## What the System Module Enables
...
programs.xwayland.enable = true;   # X11 compatibility layer

# XDG portals — COSMIC first, GTK as fallback
